...
impl<'a> System<'a> for RenderingSystem<'a> {
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        // Clearing the screen (this gives us the background color)
        graphics::clear(self.context, graphics::Color::new(0.95, 0.95, 0.95, 1.0));

        // Get all the renderables wit their positions and sort by the position z
                ...
                for x_or_y in range {
                    ...
                    match mov.get(&pos) {
                        ...
                        None => {
                            ...
                            // if it doesn't exist, we stop becaseu we found a gap
                            match immov.get(&pos) {
    ...
    for (y, row) in rows.iter().enumerate() {
        ...
        for (x, column) in columns.iter().enumerate() {
            ...
            let position = Position {
                x: x as u8,
                y: y as u8,
                z: 0, // we will get the z from the factory function
            };
